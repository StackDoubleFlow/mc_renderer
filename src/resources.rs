...
use zip::ZipArchive;
use minecraft_assets::api::{AssetPack, EnumerateResources, LoadResource, ResourceIdentifier, ResourceKind, ResourcePath};
...
impl EnumerateResources for ZipArchiveResourceProvider {
    fn enumerate_resources(
            ...
            for i in 0..zip.len() {
                ...
                if let Some(id) = dir.to_str().and_then(|dir| file.name().strip_prefix(dir)).and_then(|s| s.strip_suffix(kind.extension())) {
                    res.push(ResourceIdentifier::new_owned(kind, format!("{namespace}{id}")))
                }
            }
...
impl LoadResource for ZipArchiveResourceProvider {
    fn load_resource(&self, id: &ResourceIdentifier) -> Result<Vec<u8>, io::Error> {
        ...
        let mut zip = self.0.borrow_mut();
        let mut file = zip.by_name(path.to_str().unwrap()).map_err(|e| io::Error::new(io::ErrorKind::NotFound, e))?;
        let mut buf = Vec::new();
...
impl Resources {
    pub fn init(path: &Path) -> Result<Self> {
        ...
        dbg!(assets.load_block_model_recursive("block/oak_planks").unwrap());
        Ok(Resources {  })
    }
}
